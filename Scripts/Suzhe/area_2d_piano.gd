...
const DIALOG_PATH = "res://dialog/piano/timeline.dtl"
...
func _ready():
	...
	if not Dialogic.signal_event.is_connected(_change_to_piano):
		Dialogic.signal_event.connect(_change_to_piano)
...
func _change_to_piano(event_name: String):
	if event_name == "_change_to_piano":
		_load_new_scene()
...
func _load_new_scene():
	...
	Global.stop_music()
	get_tree().change_scene_to_file("res://Scenes/Suzhe_scenes/piano.tscn")
