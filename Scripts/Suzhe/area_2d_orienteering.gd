...
const DIALOG_PATH = "res://dialog/orienteering/timeline.dtl"
...
func _ready():
	...
	if not Dialogic.signal_event.is_connected(_change_to_orienteering):
		Dialogic.signal_event.connect(_change_to_orienteering)
...
func _change_to_orienteering(event_name: String):
	if event_name == "_change_to_orienteering":
...
func _load_new_scene():
	...
	get_tree().change_scene_to_file("res://Scenes/Suzhe_scenes/OrienteeringScenes/Orienteering.tscn")
