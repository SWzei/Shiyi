...
func start_the_round():
	...
	hint_label.text = "提示："
	apply_font_to(hint_label, "res://assets/Fonts/SourceHanSansCN-VF.otf", 48)
	hint_label.add_theme_color_override("font_color", Color(1, 1, 1))
	hint_label.autowrap_mode = TextServer.AUTOWRAP_WORD_SMART
	hint_label.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	apply_font_to(progress, "res://assets/Fonts/SourceHanSansCN-VF.otf", 36)
...
func spawn_answer_boxes():	
	...
	for i in char_count:
		...
		box.name = "Box_%d" % i
		apply_font_to(box, "res://assets/Fonts/SourceHanSansCN-VF.otf", 56)
...
func _on_Timer_timeout():
	if current_hint_index == hints[current_round_index].size() - 1:
		$Timer.stop()
		$Timer.wait_time = 25
		$Timer.start()
...
#			go_to_next_scene()
			...
			if user_answer == answer[current_round_index]:
				keyboard_lock = true
				cancel_typing = true
				apply_font_to(hint_label, "res://assets/Fonts/SourceHanSansCN-VF.otf", 40)
				$Timer.stop()
...
func _load_new_scene():
	Global.switch_scene("school_museum2")
